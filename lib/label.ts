...
export interface LabelRow {
  label: string;
  /** null when a selected item does not publish this nutrient. */
  value: number | null;
  /** true when this total includes a figure we estimated. */
  approx?: boolean;
  unit: string;
...
export function labelRows(
  ...
  const miss = (f: string) => missing?.has(f) ?? false;
  const est = (f: string) => estimated?.has(f) ?? false;
  return [
    ...
    { label: "Cholesterol", value: miss("cholesterol_mg") ? null : show(totals.cholesterol_mg, 0), unit: "mg", dv: miss("cholesterol_mg") ? undefined : pctDV("cholesterol_mg", totals.cholesterol_mg), bold: true, approx: est("cholesterol_mg") },
